    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Sliding Panel */}
          <motion.div
            className="fixed inset-x-0 bottom-0 top-auto md:right-0 md:inset-x-auto md:top-0 md:bottom-0 z-50 w-full md:max-w-[480px] max-h-[85vh] md:max-h-none h-full bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.1)] md:shadow-2xl flex flex-col font-sans origin-bottom md:origin-top-right rounded-t-3xl md:rounded-none overflow-hidden"
          ...
          >
            ...
            {/* Header */}
            <div className="px-5 pb-5 pt-2 md:pt-5 border-b border-zinc-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-5 h-5 text-[var(--color-brand-red)] hidden md:block" />
                <h2 className="font-bold text-2xl md:text-lg text-[#181211]">Your Cart</h2>
              </div>
              <div className="flex items-center gap-3">
                <div className="bg-[var(--color-brand-red)]/10 text-[var(--color-brand-red)] text-sm md:text-[10px] font-bold px-3 py-1 md:px-2 md:py-0.5 rounded-full">
                  {cartItems.length} {cartItems.length === 1 ? 'Item' : 'Items'}
                </div>
                <button
                  onClick={onClose}
                  className="p-1 rounded-full hover:bg-zinc-100 transition-colors text-zinc-400 hover:text-[#181211] hidden md:block"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
